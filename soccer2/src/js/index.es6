...
function construirEquipos(){
  arrayEquipos.forEach((e, i)=>{ equipos.push({nombre: e, escudo: escudos[i], golesPartido: 0, victorias: 0, golesFavor: 0, golesContra: 0}) })
}

function decidirPartidos(){                      //Empareja los equipos en cada ronda
    let equipoA = elegirEquipo();
    let equipoB = elegirEquipo();
